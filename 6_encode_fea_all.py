...
WORKDIR = '..\\Data\\Train_Feature\\Feature\\'
WORKDIR_Au = '..\\Data\\AU_Feature\\'
LABELDIR = '.\\VideoLabel\\VideoLabel\\VideoLabelNew\\'
#LABELDIR = '.\\VideoLabel\\VideoLabel\\VideoLabel_lev\\'
FEATUREDIR = '.\\VideoFeature\\Combine\\'
ROOT = os.getcwd()
LABELTYPE = [ 'Act']
frame_rate = 30
...
#%%
dim_PPG = ib.load('D:\\Lab\\Dennis\\Gamania\\Script\\VideoFeature\\NewFeature2\\Delta\\dim.pkl' )
dim_AU  = ib.load('D:\\Lab\\Dennis\\Gamania\\Script\\VideoFeature\\ActionUnit\\Interact+delta\\dim.pkl')
...
#            spk_most = whospeakmost(Act_label, 1)
            ...
            for key2 in sorted(fea_Com.keys()):
                ...
                for i1, tmp in enumerate(sorted(Act_label[key2])):
                    ...
                    fea2_withdelta = delta_extract(fea_Com_au[key2])
                    df_tmp, answer = select_index(fea_withdelta, float(start), float(end), frame_rate)
                    df_tmp2, answer2 = select_index(fea2_withdelta, float(start), float(end), frame_rate)
                    if answer ==1:
                        label_tmp.append(1)
                    else:
                        label_tmp.append(0)
                    ...
                    df_tmp_array2 = df_tmp.as_matrix()
                    if answer == 1 and answer2 == 1:                  
                        if len(np.where(np.isnan(df_tmp_array))[0]) > 0:
                            pdb.set_trace()
                        if len(np.where(np.isnan(df_tmp_array2))[0]) > 0:
                            pdb.set_trace()                            
                        #add delta to matrix
                        feaComCut =  getFunctional(df_tmp_array)
                        feaComCut2 = getFunctional(df_tmp_array2)
...
#                            New_feat2 = Act_feature_extend_new2(fea_Com[spk_most], float(start), float(end), 0, 5, frame_rate, feaComCut)
                        ...
                        feaComCut2 = np.append(feaComCut2, New_feat2)
                        if len(np.where(np.isnan(feaComCut))[0]) > 0:
                            pdb.set_trace()
                        
                        #Append the label to the encoded feature                           
                        feaComFinal = np.hstack((feaComCut[dim_PPG],feaComCut2[dim_AU])) 
                        feaComFinal = np.append(feaComFinal, int(lab))
